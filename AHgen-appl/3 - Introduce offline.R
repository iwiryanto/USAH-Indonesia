...
AH_scenario = "RawaTerate-offline"
...
USAH_template_baseline = 
  read_rds("data/template/3.0/current/USAH_3.0_template_baseline_IDN_20240202-154732.RDS") ## Specify master USAH with all vertex and edge information

# Read in the USAH_input you want to start with e.g. USAH_template_baseline
USAH_location_baseline = 
  read_rds("outputs/USAH_3.0_Cakung_baseline_20240202-155429.RDS") ## Specify baseline USAH you want to compare to
...
count_location_baseline = 
  read_rds("../OSMtidy-appl/outputs/Jakarta/Cakung/FINAL_count_3.0_Cakung_baseline_20240202-152258.RDS") ## Specify 

# Read in the OSMtidy count for the offline scenario
count_location_offline = 
  read_rds("../OSMtidy-appl/outputs/Jakarta/Rawa Terate/FINAL_count_3.0_RawaTerate_baseline_20240202-152836.RDS")
...
countCompared = 
  compareOSMtidy(
    ...
    countOSMtidy_scenario = count_location_offline) ## Specify offline scenario OSMtidy count
...
edgesOffline = 
  weight_hazard(
    ...
    hazard = "offline", ## Specify hazard type
...
edgelist_location_offline = 
  USAH_location_baseline$edgelist %>%
  weight_edges(edgesNew = edgesOffline, remove = FALSE) %>% # Swap in edgesNew weights for offline scenario
...
USAH_scenario = 
  apply_scenario(    
    ...
    edgelist_scenario = edgelist_location_offline, ## Specify edgelist for new scenario
    AH_name = AH_name, 
...
### edgesOffline or other edges to change can be recorded in AHgen-appl under the offline folder ###
# This is to keep a record of what types of edges were modified for this 
# offline scenario, comparable to an edgeGuide
edgesOffline %>% 
  export_AHgen(
    type = "USAH_edges",
    directory = "data/hazards/offline/",
    AH_version = AH_version, 
    AH_location = AH_location, 
    AH_scenario = AH_scenario)
